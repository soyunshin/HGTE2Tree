...
class Graph_Module(nn.Module):
	def __init__(self, indim, hiddim, outdim, dropout=0.3):
		...
		self.gtn = GTN(6, 1)
		self.graph = GCN(indim, hiddim, outdim, dropout)
...
class GTN(nn.Module):
    ...
    def __init__(self, num_edge, num_channels):
        ...
        # self.is_norm = norm
        self.layer1 = GTLayer(num_edge, num_channels, first=True)
        self.layer2 = GTLayer(num_edge, num_channels, first=False)
        # layers = []
    ...
    def normalization(self, H, batch_size):
    ...
    def norm(self, H, add=False):
        H = H.t()
        # print(H.shape)
        # print((((torch.eye(H.shape[0])==0).type(torch.FloatTensor)).cuda()).device)
    ...
    def forward(self, A):
        ...
        H,W1 = self.layer1(A)
        # print(H.shape)
        # H = self.normalization(H,H.shape[0])
        # print(H.shape)
